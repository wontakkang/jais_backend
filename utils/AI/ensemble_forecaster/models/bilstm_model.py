# BiLSTM_model.py
"""
BiLSTM (Bidirectional LSTM) 기반 시계열 예측 모델
- 순방향 + 역방향 LSTM 학습
- 하이퍼파라미터 튜닝 지원
"""
...
class BiLSTMModel:
    ...
    def build(self, X_train, y_train, rnn_units=128, dropout_rate=0.1, learning_rate=1e-3):
        ...
        # 양방향 LSTM
        x = keras.layers.Bidirectional(keras.layers.LSTM(rnn_units, return_sequences=False))(inputs)

        # 디코더 준비 (출력 시퀀스 길이만큼 복제)
        x = keras.layers.RepeatVector(output_seq_len)(x)

        # 디코더 BiLSTM
        x = keras.layers.Bidirectional(keras.layers.LSTM(rnn_units, return_sequences=True))(x)
