"""PyLSXGI: LS-XGI Protocol Implementation.
...
class Log:
    ...
    def apply_logging_config(cls, level, log_file_name):
        ...
        log_formatter = logging.Formatter(
            "%(asctime)s %(levelname)-5s %(module)s	[%(filename)s :%(funcName)s]:%(lineno)s %(message)s"
        )
    ...
    @classmethod
    def debug(cls, txt: object, *args: object) -> object:
        """Log debug messagees."""
        if cls.LOG_LEVEL == logging.NOTSET:
            cls.LOG_LEVEL = cls._logger.getEffectiveLevel()
