...
class Accel_Read_RequestBase(DE_MCU_Request):
    """
    DE-MCU 가속도계 읽기 요청 PDU 생성 클래스
    """
...
class ACCEL_READ_Request(Accel_Read_RequestBase):
    """
...
class Accel_Read_ResponseBase(DE_MCU_Response):
    """
    DE-MCU 가속도계 읽기 응답 PDU 생성 클래스
    """
...
class Accel_Read_Response(Accel_Read_ResponseBase):
    """
    DE-MCU 가속도계 읽기 응답 PDU 생성 클래스
    """
    ...
    def deserialize(self, data):
        ...
        # Map raw float axes
        roll, pitch = map(float, res_data)
        processed_data["depth"] = 3
        processed_data.setdefault("STATUS", {}).setdefault("Position", {
            "Roll": {
                "Value": roll, "Unit": "°",
            },
            "Pitch": {
