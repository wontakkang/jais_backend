...
class Digital_Output_Read_RequestBase(DE_MCU_Request):
    """
...
class Digital_Output_Read_Request(Digital_Output_Read_RequestBase):
    """
...
class Digital_Output_Read_ResponseBase(DE_MCU_Response):
    """
...
class Digital_Output_Read_Response(Digital_Output_Read_ResponseBase):
    """
    DE-MCU 노드 선택 요청 PDU 생성 클래스 (serial_number를 HEX 문자열로 입력받음)
    """
    ...
    def deserialize(self, data):
        ...
        processed_data.setdefault("STATUS", {}).setdefault("Digital_Output", {
            f"DO{int(self.req_data)}": {
                "Id": int(self.req_data), "Value": struct.unpack('B', data[3 : 4])[0], "Current": struct.unpack('<H', data[4 : 6])[0]
            }
        })
