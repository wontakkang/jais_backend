...
class Firmware_Version_Read_RequestBase(DE_MCU_Request):
    """
    DE-MCU 노드 선택 요청 PDU 생성 클래스
    """

    name = "FIRMWARE_VERSION_READ_REQ"
...
class Firmware_Version_Read_Request(Firmware_Version_Read_RequestBase):
    """
...
class Firmware_Version_Read_ResponseBase(DE_MCU_Response):
    """
    DE-MCU 노드 선택 응답 PDU 생성 클래스
    """

    name = "FIRMWARE_VERSION_READ_RES"
...
class Firmware_Version_Read_Response(Firmware_Version_Read_ResponseBase):
    """
    DE-MCU 노드 선택 요청 PDU 생성 클래스 (serial_number를 HEX 문자열로 입력받음)
    """
    ...
    def deserialize(self, data):
        ...
        if self.command != struct.unpack("<B", data[1:2])[0]:
            ...
            return None
        res_data = struct.unpack(self.command_format[3].format_string, data[3 : 3 + data_length])
        processed_data["depth"] = 2
        processed_data.setdefault("STATUS", {}).setdefault("Firmware", {
            "Version": f"{res_data[0]}.{res_data[1]}.{res_data[2]}"
        })
