import struct
import time
from .config import CommandCode, get_command_format, get_mcu_config, get_start_byte
...
class Node_Select_RequestBase(DE_MCU_Request):
    """
    DE-MCU 노드 선택 요청 PDU 생성 클래스
    """

    name = "NODE_SELECT_REQ"
    ...
    def __init__(self, **kwargs):
        ...
        # 기존 인덱싱 대신 CommandFormat 객체의 format_string을 사용하여 self.format 구성
        try:
    ...
    def serialize(self):
        ...
        try:
            # 기존 방식: 명시적 바이트 추가
            self.byte_frame += bytes([self.command]) + bytes([self.data_length]) + self.serial_number
...
class Node_Select_Request(Node_Select_RequestBase):
    """
...
class Node_Select_ResponseBase(DE_MCU_Response):
    """
    DE-MCU 노드 선택 응답 PDU 생성 클래스
    """

    name = "NODE_SELECT_RES"
...
class Node_Select_Response(Node_Select_ResponseBase):
    """
    DE-MCU 노드 선택 요청 PDU 생성 클래스 (serial_number를 HEX 문자열로 입력받음)
    """
